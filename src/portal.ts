...
export function createPortal(
  ...
  const portalEdgeGeometry = new THREE.BufferGeometry().setFromPoints(
    circleVertices
  );
  const portalEdgeMaterial = new THREE.LineBasicMaterial({ color: 0xffe500 });
  const portalEdgeMesh = new THREE.LineLoop(
    portalEdgeGeometry,
    portalEdgeMaterial
  );
  ...
  for (let i = 0; i < smokeInstances; i++) {
    ...
    const smokeMesh: THREE.Mesh = new THREE.Mesh(
      ringGeometry,
      portalSmokeMaterial
    );
  ...
  for (let i = 0; i < lightningInstances; i++) {
    const lightningAngleIncrement: number =
      circleAngleInRadians / lightningInstances;
    ...
    const lightningMesh: THREE.Mesh = new THREE.Mesh(
      ringGeometry,
      portalLightningMaterial
    );
  ...
  function updateOpacityAnimation(material: THREE.Material): void {
    ...
    const opacityValue: number =
      minOpacity +
      (opacityRange * (Math.sin(frameCounter * opacitySpeed) + 1)) / 2;
    material.opacity = opacityValue;
  }

  function updateSmokeScaling(
    ...
    const scalingFactor: number =
      smokeMinScale +
      ((smokeMaxScale - smokeMinScale) *
        (Math.sin(frameCounter * scalingSpeed) + 1)) /
        2;
  ...
  function updateLightningOpacityLoop(): void {
    ...
    for (let i = 0; i < lightningGroup.children.length; i++) {
      ...
      const opacityValue: number =
        lightningOpacityMax - lightningOpacityLoopCounter * opacityDecayRate;
  ...
  function calculateNumVisibleLightning(): number {
    ...
    const visibleLightningRange: number =
      maxVisibleLightning - minVisibleLightning;
    const numVisibleLightning: number =
      Math.floor(Math.random() * (visibleLightningRange + 1)) +
      minVisibleLightning;
  ...
  function resetLightningPositions(): void {
    lightningGroup.children.forEach(function (
  ...
  function moveLightningMeshes(): void {
    ...
    const centerPosition: THREE.Vector3 = new THREE.Vector3(
      OFFSET_X,
      0.1,
      OFFSET_Z
    );
    ...
    lightningGroup.children.forEach(function (
      ...
      const targetPosition = isMovingToCenter
        ? centerPosition
  ...
  function updatePortalEdgeAnimation(): void {
    ...
    for (let i = 0; i <= circleSegments; i++) {
      ...
      const waveOffset1: number =
        Math.sin(angle * waveFrequency1 + frameCounter * 0.01) * waveAmplitude1;
      const waveOffset2: number =
        Math.cos(angle * waveFrequency2 + frameCounter * 0.01) * waveAmplitude2;
